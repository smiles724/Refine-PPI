...
from .basis import (get_basis, get_D_to_from_z_axis)
from .reversible import (SequentialSequence, ReversibleSequence)
from .utils import (exists, default, batched_index_select, masked_mean, to_order, cast_tuple, safe_cat, slice_for_centering_y_to_x, pad_for_centering_y_to_x)
...
class Norm(nn.Module):
    @beartype
    def __init__(self, fiber: Tuple[int, ...], eps=1e-12, ):
        """
...
class DTP(nn.Module):
    ...
    @beartype
    def __init__(self, fiber_in: Tuple[int, ...], fiber_out: Tuple[int, ...], self_interaction=True, project_xi_xj=True,  # whether to project xi and xj and then sum, as in paper
                 project_out=True,  # whether to do a project out after the "tensor product"
                 pool=True, edge_dim=0, radial_hidden_dim=16):
        super().__init__()
        ...
        for degree_out, dim_out in enumerate(self.fiber_out):
            ...
            for degree_in, (dim_in, dim_out_from_degree_in) in enumerate(zip(self.fiber_in, split_dim_out)):
                ...
                self.kernel_unary[f'({degree_in},{degree_out})'] = Radial(degree_in, dim_in, degree_out, dim_out_from_degree_in, radial_hidden_dim=radial_hidden_dim,
                                                                          edge_dim=edge_dim)
    ...
    @beartype
    def forward(self, inp, basis, D, edge_info: EdgeInfo, rel_dist=None, ):
...
class Radial(nn.Module):
    def __init__(self, degree_in, nc_in, degree_out, nc_out, edge_dim=0, radial_hidden_dim=64):
        super().__init__()
        ...
        self.rp = nn.Sequential(nn.Linear(edge_dim + 1, mid_dim), nn.SiLU(), LayerNorm(mid_dim), nn.Linear(mid_dim, mid_dim), nn.SiLU(), LayerNorm(mid_dim),
                                nn.Linear(mid_dim, nc_in * nc_out), Rearrange('... (lo li) -> ... lo li', li=nc_in, lo=nc_out))
...
class FeedForward(nn.Module):
    @beartype
    def __init__(self, fiber: Tuple[int, ...], fiber_out: Optional[Tuple[int, ...]] = None, mult=4, include_htype_norms=True, init_out_zero=True):
        super().__init__()
        self.fiber = fiber
...
class L2DistAttention(nn.Module):
    @beartype
    def __init__(self, fiber: Tuple[int, ...], dim_head: Union[int, Tuple[int, ...]] = 64, heads: Union[int, Tuple[int, ...]] = 8, attend_self=False, edge_dim=None,
                 single_headed_kv=False, radial_hidden_dim=64, num_linear_attn_heads=0, init_out_zero=True):
        super().__init__()
    ...
    def forward(self, features, edge_info: EdgeInfo, rel_dist, basis, D, mask=None):
        ...
        keyvalues = self.to_kv(features, edge_info=edge_info, rel_dist=rel_dist, basis=basis, D=D)
...
class MLPAttention(nn.Module):
    @beartype
    def __init__(self, fiber: Tuple[int, ...], dim_head: Union[int, Tuple[int, ...]] = 64, heads: Union[int, Tuple[int, ...]] = 8, attend_self=False, edge_dim=None,
                 single_headed_kv=False, attn_leakyrelu_slope=0.1, attn_hidden_dim_mult=4, radial_hidden_dim=16, num_linear_attn_heads=0, init_out_zero=True, **kwargs):
        super().__init__()
        num_degrees = len(fiber)
        ...
        self.to_attn_logits = nn.ModuleList(
            [nn.Sequential(nn.LeakyReLU(attn_leakyrelu_slope), nn.Linear(attn_hidden_dim, h, bias=False)) for attn_hidden_dim, h in zip(attn_hidden_dims, self.heads)])

        # non-linear transform of the value branch
        # todo - needs a DTP here?

        self.to_values = nn.Sequential(Gate(value_gate_fiber), Linear(value_hidden_fiber, value_hidden_fiber))
    ...
    def forward(self, features, edge_info: EdgeInfo, rel_dist, basis, D, mask=None):
        ...
        intermediate = self.to_attn_and_v(features, edge_info=edge_info, rel_dist=rel_dist, basis=basis, D=D)
...
class Equiformer(nn.Module):
    @beartype
    def __init__(self, *, dim: Union[int, Tuple[int, ...]], dim_in: Optional[Union[int, Tuple[int, ...]]] = None, num_degrees=2, input_degrees=1,
                 heads: Union[int, Tuple[int, ...]] = 8, dim_head: Union[int, Tuple[int, ...]] = 24, depth=1, valid_radius=1e5, num_neighbors=float('inf'), reduce_dim_out=False,
                 radial_hidden_dim=64, num_tokens=None, num_positions=None, num_edge_tokens=None, edge_dim=None, attend_self=True, linear_out=True,
                 embedding_grad_frac=0.5, single_headed_kv=False,  # whether to do single headed key/values for dot product attention, to save on memory and compute
                 ff_include_htype_norms=False,
                 # whether for type0 projection to also involve norms of all higher types, in feedforward first projection. this allows for all higher types to be gated by other type norms
                 l2_dist_attention=True,
                 # turn to False to use MLP attention as proposed in paper, but dot product attention with -cdist similarity is still far better, and i haven't even rotated distances (rotary embeddings) into the type 0 features yet
                 ...
                 num_adj_degrees_embed=None, adj_dim=0, max_sparse_neighbors=float('inf'), **kwargs):
        super().__init__()
        ...
        # main network
        self.tp_in = DTP(self.dim_in, self.dim, edge_dim=edge_dim, radial_hidden_dim=radial_hidden_dim)

        # trunk
        self.layers = []
        attention_klass = L2DistAttention if l2_dist_attention else MLPAttention
        for ind in range(depth):
            self.layers.append((attention_klass(self.dim, heads=heads, dim_head=dim_head, attend_self=attend_self, edge_dim=edge_dim, single_headed_kv=single_headed_kv,
                                                radial_hidden_dim=radial_hidden_dim, **kwargs), FeedForward(self.dim, include_htype_norms=ff_include_htype_norms)))
    ...
    @beartype
    def forward(self, inputs, coors, mask=None, adj_mat=None, edges=None, return_pooled=False):
        ...
        # project in
        x = self.tp_in(x, edge_info=edge_info, rel_dist=neighbor_rel_dist, basis=self.basis, D=D)

        # transformer layers
        attn_kwargs = dict(edge_info=edge_info, rel_dist=neighbor_rel_dist, basis=self.basis, D=D, mask=_mask)
