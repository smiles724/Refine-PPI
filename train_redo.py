...
if __name__ == '__main__':
    ...
    def train(it):
        ...
        for k, v in loss_dict.items():
            logstr += ' | %s %.2f' % (k, v.item() * config.train.loss_weights[k] if isinstance(v, torch.Tensor) else v * config.train.loss_weights[k])
    ...
    def validate(it, best_it, best_loss):
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(val_loader, desc='Validate', dynamic_ncols=True)):
                ...
                loss_dict = model(batch, mode='val')
                loss = sum_weighted_losses(loss_dict, config.train.loss_weights)
    ...
    try:
        ...
        for it in it_tqdm:
            ...
            if it % config.train.val_freq == 0:
                avg_val_loss, best_loss, best_i = validate(it, best_i, best_loss)
                if not args.debug and best_i == it:
