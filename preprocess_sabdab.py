...
from rde.utils.protein import constants, parsers
...
def preprocess_sabdab_structure(task, ag_len_threshold=20, cutoff=10.0, sasa=False):
    ...
    try:
        ...
        if len(entry['ag_chains']) == 0:
            raise ValueError(f'No antigen chain is found.')
...
def _preprocess_structures(path, ):
    data_list = joblib.Parallel(n_jobs=n_jobs)(joblib.delayed(preprocess_sabdab_structure)(task, sasa=False) for task in tqdm(tasks, dynamic_ncols=True, desc='Preprocess'))
