...
class FocusedRandomPatch(object):
    ...
    def __call__(self, data):
        ...
        nbh_seed_idx = dist_from_seed.argsort()[:self.seed_nbh_size]  # (Nb, ), neighbourhood
...
class SelectedInterfaceRegionPaddingPatch(object):
    ...
    def __init__(self, fix_size=128, cutoff=None, ):
        super().__init__()
        self.cutoff = cutoff
    ...
    def __call__(self, data):
        pos_CB = _get_CB_positions(data['pos_atoms'], data['mask_atoms'])  # (L, 3)
        ag_idx = [i for i in range(len(data['fragment_type'])) if data['fragment_type'][i] == Fragment.Antigen]
        ab_idx = [i for i in range(len(data['fragment_type'])) if data['fragment_type'][i] != Fragment.Antigen]
        ...
        if self.cutoff:
            ...
            assert len(ag_id_selected) > 0 and len(ab_id_selected) > 0, f'no residue selected. Min dist: {torch.min(dist)}. Enlarge the threshold!'
        else:
            dist_from_ab = dist.min(dim=1)[0]   # (AG, )
            dist_from_ag = dist.min(dim=0)[0]   # (AB, )
            ag_id_selected = [ag_idx[i] for i in torch.topk(dist_from_ab, self.fix_size // 2, largest=False)[1]]
            ab_id_selected = [ab_idx[i] for i in torch.topk(dist_from_ag, self.fix_size // 2, largest=False)[1]]
