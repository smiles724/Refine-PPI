...
class CorruptChiAngle(object):
    ...
    def __call__(self, data):
        chi_native = torch.where(torch.randn_like(data['chi']) > 0, data['chi'], data['chi_alt'], )  # (L, 4), randomly pick from chi and chi_alt
...
class CorruptChiAngle(object):
    ...
    def _get_flip_prob(self, min_dist):
        return torch.where(min_dist <= 8.0, torch.full_like(min_dist, 0.25), torch.zeros_like(min_dist, ), )
    ...
    def _random_flip_chi(self, chi, flip_prob, chi_mask):
        ...
        Args:
            ...
            chi_mask: (L, 4)
        """
        delta = torch.where(torch.rand_like(chi) <= flip_prob[:, None], torch.full_like(chi, np.pi), torch.zeros_like(chi), ) * chi_mask
    ...
    def __call__(self, data):
        ...
        if self.maskable_flag_attr is not None:
            ...
            idx = idx[flag]
            assert len(idx) > 0, 'No available residues!'

        if self.ratio_mask < 1.0:
            idx = idx.tolist()
            np.random.shuffle(idx)
            idx_mask = idx[:num_mask]    # (L)
        else:
            idx_mask = idx.tolist()
        ...
        corrupt_flag[min_dist <= 8] = True   # predict neighboring residues but not masking them
