...
class ScalarMetricAccumulator(object):
    ...
    def log(self, it, tag, best_it=None, best_metric=None, best_loss=None, logger=BlackHole(), writer=BlackHole()):
        ...
        if best_metric is not None:
            logstr += ' | Best iter %05d | Best spearman (PC) %.4f' % (best_it, best_metric)
...
def sum_weighted_losses(losses, weights, echo=False):
    """
    Args:
    ...
    for k in losses.keys():
        if weights is None:
            loss = loss + losses[k]
        elif k in weights.keys():
            loss = loss + weights[k] * losses[k]
        elif echo:
            print(f'Warning: no weight for {k}.')
    return loss
...
def aggregate_sidechain_accuracy(aa, chi_pred, chi_native, chi_mask):
    ...
    diff = torch.min((chi_pred - chi_native) % (2 * np.pi), (chi_native - chi_pred) % (2 * np.pi), )  # (N, L, 4)
    diff = torch.rad2deg(diff)
    diff = diff.reshape(-1, 4)

    diff_flip = torch.min(((chi_pred + np.pi) - chi_native) % (2 * np.pi), (chi_native - (chi_pred + np.pi)) % (2 * np.pi), )
...
class CrossValidation(object):
    ...
    def __init__(self, model_factory, config, num_cvfolds, direct_tune=False):
        super().__init__()
        self.num_cvfolds = num_cvfolds

        # Loading pretrained model weights
        if direct_tune and config.model.checkpoint.path:
            ckpt = torch.load(config.model.checkpoint.path, map_location='cpu')
            self.models = [model_factory(ckpt['config'].model, fine_tune=True) for _ in range(num_cvfolds)]
            for i in range(num_cvfolds):
                print(f'Loading from {config.model.checkpoint.path}')
    ...
    def state_dict(self):
        return {'models': [m.state_dict() for m in self.models], 'optimizers': [o.state_dict() for o in self.optimizers], 'schedulers': [s.state_dict() for s in self.schedulers], }
