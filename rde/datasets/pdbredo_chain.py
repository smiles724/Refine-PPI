...
from rde.utils.transforms._base import _truncate_data
...
class PDBRedoChainDataset(Dataset):
    ...
    def __init__(self, split, pdbredo_dir='./data/PDB_REDO', clusters_path='./data/pdbredo_clusters.txt', splits_path='./data/pdbredo_splits.txt',
                 processed_dir='./data/PDB_REDO_processed', num_preprocess_jobs=math.floor(cpu_count() * 0.8), iter_='cluster', use_plm=False, transform=None, reset=False, ):
        super().__init__()
        ...
        self.plm_path = os.path.join(self.processed_dir, 'embeddings_output_10000')   # max sequence length = 10K
        ...
        if self.iter_ != 'cluster':
            self.pdbchains = []
            for clust_name, pdbchain_list in self.clusters.items():
                if clust_name in self.splits[split]:
                    for pdbcode, chain in pdbchain_list:
                        self.pdbchains.append([clust_name, pdbcode, chain])
        else:
    ...
    def _preprocess_structures(self, reset):
        ...
        for pdbcode in pdbcodes:
            cif_path = os.path.join(self.pdbredo_dir, pdbcode[1:3], pdbcode, f"{pdbcode}_final.cif")
    ...
    def __len__(self):
        if self.iter_ != 'cluster':
            return len(self.pdbchains)
        return len(self._clusters_of_split)

    def __getitem__(self, index):
        if self.iter_ != 'cluster':
            clust_name, pdbcode, chain = self.pdbchains[index]
        else:
        ...
        if self.use_plm:
            ...
            if len(data['aa']) > len(data['plm_wt']):  # ESM-2 features are truncated
                data = _truncate_data(data, len(data['plm_wt']))
                chain = random.choice(list(set(data['chain_id'])))
...
def get_pdbredo_chain_dataset(split, cfg, iter_='cluster', use_plm=False):
    from rde.utils.transforms import get_transform
    return PDBRedoChainDataset(split=split, pdbredo_dir=cfg.pdbredo_dir, clusters_path=cfg.clusters_path, splits_path=cfg.splits_path, processed_dir=cfg.processed_dir,
                               transform=get_transform(cfg.transform), iter_=iter_, use_plm=use_plm)
...
if __name__ == '__main__':
    ...
    print(f'[INFO] {len(dataset)} samples in the {args.split} split.')
