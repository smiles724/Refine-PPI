...
from rde.utils.misc import seed_all
from rde.utils.protein.constants import (AA, three_to_one, non_standard_residue_substitutions)
...
if __name__ == "__main__":
    ...
    parser.add_argument('data', type=str, choices=['skempi', 'redo', 't50'])
    ...
    parser.add_argument("--split", default='train', choices=['train', 'val'], help="For T50 only.")
    ...
    if args.data == 'skempi':
        ...
        structures_cache = os.path.join(cache_dir, f'structures_{args.split}.pkl')
        entries_cache = os.path.join(cache_dir, f'entries_{args.split}.pkl')
        fasta_cache = os.path.join(cache_dir, f'sequences_{args.split}.fasta')
    ...
    if not os.path.exists(fasta_cache):
        ...
        if args.data == 'redo':
            ...
            if args.data == 'skempi':
                ...
                for entry in entries_full:
                    idx = entry['pdb_path']
                    data = copy.deepcopy(structures[idx])
    ...
    run(args)
    if args.data != 'redo':     # REDO is too large to save in one file
        if args.data == 'skempi':
            output_path = os.path.join(cache_dir, 'esm2_embeddings.pt')
        else:
            output_path = os.path.join(cache_dir, f'esm2_embeddings_{args.split}.pt')
