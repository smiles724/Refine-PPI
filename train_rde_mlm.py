...
if __name__ == '__main__':
    ...
    model = MaskedLanguageModelingDensityEstimator(config.model).to(args.device)
    ...
    def validate(it):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(val_loader, desc='Validate', dynamic_ncols=True)):
                ...
                xs = model.sample(batch)
                chi_pred.append(xs.cpu())
                chi_native.append(batch['chi_native'].cpu())
                chi_masked_flag.append((batch['chi_masked_flag'][..., None] * batch['chi_mask']).cpu())
                chi_corrupt_flag.append((batch['chi_corrupt_flag'][..., None] * batch['chi_mask']).cpu())
    ...
    try:
        ...
        for it in it_tqdm:
            ...
            # Logging
            scalar_dict = {'grad': orig_grad_norm, 'lr': optimizer.param_groups[0]['lr'], 'time_forward': (time_forward_end - time_start) / 1000,
            ...
            it_tqdm.set_description(logstr)
